...
The free tier is 100 requests/month. Each run spends:
  - 1 request per (keyword x source) on the watchlist
  - 1 request per discovery feed
This module projects monthly usage from the config + how often the workflow runs,
and tells the user to slow down or upgrade if they'd blow the cap.
"""
...
def print_report(cfg: Config) -> None:
    per_run = cfg.requests_per_run()
    watch_calls = sum(len(w.sources) for w in cfg.watchlist)
    feed_calls = len(cfg.discovery_feeds)

    print("TrendWatch quota estimate")
    print("=" * 40)
    print(f"Watchlist calls per run : {watch_calls}  ({len(cfg.watchlist)} keyword(s))")
    print(f"Discovery calls per run : {feed_calls}  ({len(cfg.discovery_feeds)} feed(s))")
